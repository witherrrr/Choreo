...
SwerveTrajectoryGenerator::SwerveTrajectoryGenerator(
  ...
  struct ModuleVelocityKinematics {
    slp::Variable<double> v_norm;
    slp::Variable<double> friction_factor;
    slp::Variable<double> V_emf;
    Translation2v<double> v_wheel_wrt_robot;
  };
  struct SampleVelocityKinematics {
    Rotation2v<double> neg_θ;
    std::vector<ModuleVelocityKinematics> modules;
  };

  auto compute_sample_velocity_kinematics =
      [&](const Translation2v<double>& v_world,
          const slp::Variable<double>& ω_val,
          const Rotation2v<double>& θ_val) {
        Rotation2v<double> neg_θ = -θ_val;
        auto v_wrt_robot = v_world.rotate_by(neg_θ);
        SampleVelocityKinematics kin{neg_θ, {}};
        kin.modules.reserve(path.drivetrain.modules.size());
        for (const auto& translation : path.drivetrain.modules) {
          ...
          kin.modules.push_back(
              {v_norm, friction_factor, V_emf, v_wheel_wrt_robot});
        }
        return kin;
      };
      ...
      [&](size_t force_index, const SampleVelocityKinematics& kin) {
        for (size_t module_index = 0;
          ...
          auto I_motor = (F_longitudinal + F_drag) / kT_over_r +
                         I_free * mk.friction_factor;
          auto V_motor = I_motor * R + mk.V_emf;
          ...
          problem.subject_to(
              F_dot_v + I_stator_limit * kT_over_r * mk.v_norm >= 0);
  ...
  // Build velocity kinematics once per sample point, then reuse for both the
  // interval ending at that sample and the interval starting at it.
  std::vector<SampleVelocityKinematics> sample_kin;
  sample_kin.reserve(samp_tot);
  for (size_t index = 0; index < samp_tot; ++index) {
    Rotation2v<double> θ_k{cosθ.at(index), sinθ.at(index)};
    Translation2v<double> v_k{vx.at(index), vy.at(index)};
    sample_kin.push_back(
        compute_sample_velocity_kinematics(v_k, ω.at(index), θ_k));
  }
